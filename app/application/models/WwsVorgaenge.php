...
class Model_WwsVorgaenge extends MyProject_Model_Database
{
    protected $_storageName = 'wwsVorgaenge';
...
//            echo "Der ist vom Cache!\n\n";
    ...
    public function query($filters, $sort, $limit, $opts) 
    {
        ...
        $stgBK = new Model_Db_Bestellkoepfe;
        $stgAP = new Model_Db_Auftragspositionen;
        $stgBP = new Model_Db_Bestellpositionen;
        
        $tblBK = $stgBK->info(Zend_Db_Table::NAME);
        $tblAP = $stgAP->info(Zend_Db_Table::NAME);
        $tblBP = $stgBP->info(Zend_Db_Table::NAME);
...
//            'mandant' => 10,
        ...
        // Entscheidende Weiche für Datebankabfrage
        $ANR = 0;
        if ($filters) {
...
//            die( '#' . __LINE__ . ' ' . basename(__FILE__) . ' jsonFilter: (src:'.print_r($filters,1).')' . print_r( $jsonFilter, 1));
        ...
        if ($ANR) { // && preg_match('/eq|=/', $ANR_OPER) ) {
            ...
            $sqlWhere = 'A.Bearbeitungsstatus BETWEEN 2 AND 9 ';
