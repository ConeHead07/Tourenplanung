 ...
 * @author rybka
 */
class Model_TourenDispoFuhrpark extends Model_TourenDispoResourceAbstract
{
    protected $_storageName   = 'tourenDispoFuhrpark';
    protected $_resourceName  = 'Fuhrpark';
    protected $_resourceModel = 'Model_Fuhrpark';
    protected $_resourceType  = 'FP';
    protected $_prmRsrcKey    = 'fid';
    ...
    {        
        ...
        foreach($rows as $_id => $_row) {
            ...
            if ($record) $record->setFromArray(array(
    ...
    public function xcountUnerfassteZeitenByTourId($tourid) {
        $sql = 'SELECT SUM( IF(einsatzdauer IS NULL AND km IS NULL, 1, 0)) '
             . ' FROM ' . $this->_db->quoteIdentifier($this->_tbl) . ' tr '
             . ' JOIN mr_fuhrpark r ON (r.'.$this->_prmRsrcKey.' = tr.fuhrpark_id) '
             . ' WHERE tour_id = :tourid';
        
        $cnt = $this->_db->fetchOne($sql, array('tourid' => $tourid));
        if ($cnt) echo '//#' . __LINE__ . ' ' . __METHOD__ . ' '.$cnt.' Treffer => sql: ' . str_replace(':tourid', $tourid, $sql) . '<br>' . PHP_EOL;
            ...
            )
    {
        ...
        $modelR = MyProject_Model_Database::loadModel('fuhrpark');
        ...
        $tblTR = MyProject_Model_Database::loadModel('tourenDispoFuhrpark')->getStorage()->info(Zend_Db_Table::NAME);
        ...
        $tblCL = MyProject_Model_Database::loadModel('fuhrparkCategoriesLnk')->getStorage()->info(Zend_Db_Table::NAME);
        $tblC  = MyProject_Model_Database::loadModel('fuhrparkCategories')->getStorage()->info(Zend_Db_Table::NAME);
        ...
        $sql = ' FROM ' . $tblR . ' r ' . PHP_EOL
             ...
             . ' LEFT JOIN ' . $tblCL . ' cl ON(r.' . $key . ' = cl.fuhrpark_id) ' . PHP_EOL
    ...
    public function getFreeResources($prmFilter, $prmPager = array() ) 
    {
        ...
        $tblLinkModelName = 'fuhrparkCategoriesLnk';
        $tblRsrcModelName = 'fuhrpark';
