 ...
 * @author rybka
 */
class MyProject_Model_Treeview {
    //put your code here
    
    protected $_table       = 'nested_category';
    ...
    {
        /* @var $storage Model_Db_Treeview */
        
        /* @var $select Zend_Db_Table_Select */
        $select = $this->select();
        $select = $this->select();
        $select->columns(array(
                    'CONCAT( REPEAT(\'-\', COUNT(parent.' . $this->_nameFld . ') - 1)', 
                    'node.' . $this->_nameFld . ') AS name '
                ))
...
EOT;
     ...
     * @return array
     */
    public function getAddNodeAsChild($newNodeName = 'FRS', $posAfterNodeName = '2 WAY RADIOS')
    {
        $sqls = array();
