...
class JqGrid extends JqGridBaseObject
{
	...
	*/
	...
	public function addChainMethod($code)
	{
        ...
        /**
         *
         * @param JqGridNavGridButton $button
         * @return JqGrid 
         */
        public function addNavButton(JqGridNavGridButton $button)
        {
        ...
        public function getJsCode()
	{
		...
		if ($this->_navGrid instanceof JqGridNavGrid) {
			...
			$JsCode.= JqGridHelper::json_encode($this->_navGrid->getInstanceViewFormOptions()).')'."\n";
                        ...
                        $buttons = $this->_navGrid->getNavButtons();
                        /* @var $button JqGridNavGridButton */
                        foreach($buttons as $button) {
                            if ($button) $JsCode.= $button->render();
                        }
                }
		// Add more Chains
		$JsCode.= (string)$this->_chainMethods;
                ...
                // Add NavGridButtons
                /* @var $button JqGridNavGridButton */
                foreach($this->_navButtons as $button) {
                    if (!$button) continue;
                    $defaultID = (!$button->get_pagerID() ? $this->_navGrid->get_id() : '');
                    $JsCode.= $button->render($defaultID);
                }
		$JsCode.= ';'."\n";
		return $JsCode;
	}
	 ...
	 * Wenn chainBindung aktiviert ist, kï¿½nnen die mit JqGridMethods generierten Methodenaufrufe
	 * an die JqGrid-Funktion addChainMethod ï¿½bergeben. Beim Rendern des Grid-Js-Codes werden
	 * diese Methoden automatisch kettenfï¿½rmig angehï¿½ngt.<br>
	 * 
	...
	public function getInstanceNavGrid($navGridId = '', $properties = NULL) 
        {
            if ($navGridId && !is_object($this->_navGrid)) $this->_navGrid = new JqGridNavGrid($this->_id, $navGridId);
            if (is_object($this->_navGrid) && !is_null($properties)) $this->_navGrid->setProperties($properties);
	 ...
	 * @see JqGridPager
	 */
	public function setNavGrid($navGridId, $properties = NULL) 
        {
		$this->_navGrid = new JqGridNavGrid($this->_id, $navGridId);
	...
	*/
	 ...
	 * @abstract Determines the language direction in grid. The default is ï¿½ltrï¿½ (Left To Right language). 
	 * When set to ï¿½rtlï¿½  (Right To Left) the grid automatically do the needed. 
	 * It is important to note that in one page we can have two (or more) grids where the one can 
	 * have direction ï¿½ltrï¿½ while the other can have direction ï¿½rtlï¿½. This option work only in FireFox 3.x versions 
	 * and Internet Explorer versions &gt;=6. Currently Safai and Google Chrome does not support fully direction ï¿½rtlï¿½. 
	 ...
	 * @abstract Zum Erstellen spezielleren Verhaltens siehe Verweise
         * @param JsFunction $editEventHandler
	 ...
	 * @return JqGrid
	 */
	public function enableInlineEditingDefaults($editOpts = null, $saveOpts = null, $restoreOpts = null) 
	{       
                static $defEditPrms = array(
                    'keys' => 'true', // Aktiviert Tasten zur Steuerung (Enter=Save,ESC=Cancel)
                    'oneditfunc' => /*'null',*/'function(re)  { var re = (re.responseText) ? $.parseJSON( re.responseText ) : re; if (re.err) alert(re.err); return (typeof(re)=="object" && re.type !== \'error\' && re.success !== false );}',
                    'successfunc' => 'function(re)  { var re = (re.responseText) ? $.parseJSON( re.responseText ) : re; if (re.err) alert(re.err); return (typeof(re)=="object" && re.type !== \'error\' && re.success !== false);}',
                    'aftersavefunc' => 'null',
                    'errorfunc' => 'null',
                    'afterrestorefunc' => 'null',
                );
                
                // jqGrid('editRow',rowid, keys, oneditfunc, succesfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc)
                static $defSavePrms = array( 
                    'successfunc' => 'function(re)  { var re = (re.responseText) ? $.parseJSON( re.responseText ) : re; if (re.err) alert(re.err); return (typeof(re)=="object" && re.type !== \'error\' && re.success !== false);}',
                    'url' => 'null',
                    'extraparam' => 'null', 
                    'aftersavefunc' => 'null',
                ...
                // jqGrid('saveRow',rowid, succesfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc);
                
                static $defRestorePrms = array(
                    'afterrestorefunc' => 'null',
                );
                
                $editOpts      = (is_array($editOpts)    ? array_merge($defEditPrms, $editOpts)      : $defEditPrms);
                $saveOpts      = (is_array($saveOpts)    ? array_merge($defSavePrms, $saveOpts)      : $defSavePrms);
                $restoreOpts   = (is_array($restoreOpts) ? array_merge($defRestorePrms, $restoreOpts): $defRestorePrms);
                if (!empty($editOpts['successfunc'])) $editOpts['succesfunc'] = $editOpts['successfunc'];
                if (!empty($saveOpts['successfunc'])) $saveOpts['succesfunc'] = $saveOpts['successfunc'];
                
                $editParams    = '';
                $saveParams    = '';
                $restoreParams = '';
                
                foreach($editOpts as $k => $v)    $editParams   .= ($editParams?','   :'') . '"'.$k.'":' . (string)$v;
                foreach($saveOpts as $k => $v)    $saveParams   .= ($saveParams?','   :'') . '"'.$k.'":' . (string)$v;
                foreach($restoreOpts as $k => $v) $restoreParams.= ($restoreParams?',':'') . '"'.$k.'":' . (string)$v;
...
//				$( this ).jqGrid("saveRow", lastSel, {' . $saveParams . ');
				$( this ).jqGrid("restoreRow", lastSel, {' . $restoreParams . '});
			}
			...
			if (!lastSel || lastSel != id) {
				...
				$( this ).jqGrid("restoreRow", id, {' . $restoreParams . '});
	 ...
	 * For a full description and default setting, see Retrieving Data JSON Data 
         * <a href="http://www.trirand.com/jqgridwiki/doku.php?id=wiki:retrieving_data#json_data"> [link]</a>
	 * Default
	 * Changeable No
	 * @param array
	 * @return JqGrid
	 ...
	 * <strong>enable</strong> (default) - enables ï¿½Loadingï¿½ message in the center of the grid. <br>
	 * 
	 * <strong>block</strong> - enables the ï¿½Loadingï¿½ message and blocks all actions
	...
	/**
	 * Defines the type of request to make (ï¿½POSTï¿½ or ï¿½GE
	 * @abstract Defines the type of request to make (ï¿½POSTï¿½ or ï¿½GETï¿½)
	 * Default GET
	 * Changeable Yes
	 * @param string
	 * @return JqGrid
	 ...
	 * in our example we gave the div the id of ï¿½pagerï¿½, 
	 * but any name is acceptable. Note that the Navigation layer (the ï¿½pagerï¿½ div) can be 
	 ...
	 * @abstract  Default values prmNames: <br>
	 ...
	 {page:ï¿½pageï¿½,rows:ï¿½rowsï¿½, sort: ï¿½sidxï¿½, order: ï¿½sordï¿½, search:ï¿½_searchï¿½, nd:ï¿½ndï¿½, id:ï¿½idï¿½, oper:ï¿½operï¿½, editoper:ï¿½editï¿½, addoper:ï¿½addï¿½, deloper:ï¿½delï¿½, subgridid:ï¿½idï¿½, npage: null, totalrows:ï¿½totalrowsï¿½} <br>
	 Customizes names of the fields sent to the server on a Post. For example, with this setting, you can change the sort order element from ï¿½sidxï¿½ to ï¿½mysortï¿½: prmNames: {sort: ï¿½mysortï¿½}. The string that will be posted to the server will then be myurl.php?page=1&amp;rows=10&amp;mysort=myindex&amp;sord=asc  rather than myurl.php?page=1&amp;rows=10&amp;sidx=myindex&amp;sord=asc <br>
	...
	*/
	...
	public function subGridModel($param)
	{
         ...
         *    colModel: [ ... ],
         * }
         * @param JsFunction $param
         * @return JqGrid 
         */
        public function subGridRowExpanded($param)  {
	 ...
	 * Possible values ï¿½topï¿½,ï¿½bottomï¿½, ï¿½bothï¿½. <br>
	 * When we set toolbar: [true,ï¿½bothï¿½] two toolbars are created ï¿½ 
	 ...
	 * The id of the top bar is constructed like ï¿½t_ï¿½+id of the grid and the bottom toolbar the id is ï¿½tb_ï¿½+id of the grid. 
	 * In case when only one toolbar is created we have the id as ï¿½t_ï¿½ + id of the grid, 
	 ...
	 * The id of the new created pager is a combination of the gridid+ï¿½_toppagerï¿½.
	 * Default false
	 * Changeable No
	 * @param boolean
	 * @return JqGrid
	...
	*/
	...
	{
		$this->afterInsertRow = $param; return $this;
	}
	...
	{
		$this->beforeProcessing = $param; return $this;
	}
	...
	{
		$this->beforeRequest = $param; return $this;
	}
	...
	{
		$this->beforeSelectRow = $param; return $this;
	}
	...
	{
		$this->gridComplete = $param; return $this;
	}
	...
	{
		$this->loadBeforeSend = $param; return $this;
	}
	...
	{
		$this->loadComplete = $param; return $this;
	}
	 ...
	 * @return JqGrid
	 */
	public function on_loadError(JsFunction $param)
	{
                $this->loadError = $param; 
                return $this;
	}
	 ...
	 *
	 *  (Note that this is available when we not use cell editing module and is disabled when using cell editing).
	 *
	 * @param JsFunction
	 * @return JqGrid
	 */
	...
	{
		$this->onCellSelect = $param; return $this;
	}
	...
	{
		$this->ondblClickRow = $param; return $this;
	}
	...
	{
		$this->onHeaderClick = $param; return $this;
	}
	...
	{
		$this->onPaging = $param; return $this;
	}
	...
	{
		$this->onRightClickRow = $param; return $this;
	}
	...
	{
		$this->onSelectAll = $param; return $this;
	}
	...
	{
		$this->onSelectRow = $param; return $this;
	}
	...
	{
		$this->onSortCol = $param; return $this;
	}
	...
	{
		$this->resizeStart = $param; return $this;
	}
	...
	{
		$this->resizeStop = $param; return $this;
	}
	...
	{
		$this->serializeGridData = $param; return $this;
	}
